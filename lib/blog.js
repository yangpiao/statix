...
module.exports = {
  ...
  render: function(output, callback) {
    var posts = this.posts, me = this, postsByTags = { 'no-tag': [] };
    ...
    async.waterfall([
      ...
      function(pages, next) {
        ...
        posts.forEach(function(post, index) {
          ...
          if (!tags || !tags.length) {
            postsByTags['no-tag'].push(post);
          } else {
            for (i = 0, len = tags.length; i < len; i++) {
              tag = tags[i];
              if (!postsByTags[tag]) postsByTags[tag] = [];
              postsByTags[tag].push(post);
            }
          }
        });
        me.postsByTags = postsByTags;
        me.tags = Object.keys(postsByTags);
